...
using EnviousBox.Cli.Tools.Round.Helpers;
using EnviousBox.Cli.Tools.Round.Settings;

using Spectre.Console;
using Spectre.Console.Cli;

namespace EnviousBox.Cli.Tools.Round.Commands;
...
internal class RoundSignificantFiguresCommand : Command<RoundSignificantFiguresCommand.Settings>
{
    public override int Execute(CommandContext context, Settings settings)
    {
        ...
        {
            AnsiConsole.WriteException(new NullReferenceException(Resources.Apps_Round_Error_Number_NoInput), ExceptionFormats.ShowLinks);
            return -1;
        }
        ...
        if (!settings.PrettyMode)
        {
            ...
            Text providedValueLabel = new Text($"{Resources.Apps_Round_Input_ProvidedValue}:", new Style(Color.IndianRed)).LeftJustified();
            ...
            {
                providedPrecisionLabel = new Text($"{Resources.Apps_Round_Input_Rounding_SignificantFigures}:", new Style(Color.IndianRed)).LeftJustified();
            }
            else
            {
                providedPrecisionLabel = new Text($"{Resources.Apps_Round_Input_Rounding_SignificantFigures_NotProvided}:", new Style(Color.IndianRed)).LeftJustified();
            }
            ...
            Text resultLabel = new Text($"{Resources.Apps_Round_Input_RoundedValue}:", new Style(Color.IndianRed)).LeftJustified();
